...
#define BLE_PART1_OFFSET 0
#define BLE_PART2_OFFSET (BLE_PART1_OFFSET + BLE_PART1_BYTE_SIZE)
#define BLE_PART3_OFFSET (BLE_PART2_OFFSET + BLE_PART2_BYTE_SIZE)

#define TOTAL_BLE_BUFFER_SIZE (BLE_PART1_BYTE_SIZE + BLE_PART2_BYTE_SIZE + BLE_PART3_BYTE_SIZE)

uint8_t timestamp_adv_payload_buffer[TOTAL_BLE_BUFFER_SIZE];

static inline void set_timestamp_data_element(int idx, uint32_t val) {
    uint8_t *dest;
    if (idx < BLE_PART1_ELEMENTS) {
        dest = &timestamp_adv_payload_buffer[BLE_PART1_OFFSET + 2 + idx * 3];
    } else if (idx < BLE_PART1_ELEMENTS + BLE_PART2_ELEMENTS) {
        dest = &timestamp_adv_payload_buffer[BLE_PART2_OFFSET + 2 + (idx - BLE_PART1_ELEMENTS) * 3];
    } else {
        dest = &timestamp_adv_payload_buffer[BLE_PART3_OFFSET + 2 + (idx - BLE_PART1_ELEMENTS - BLE_PART2_ELEMENTS) * 3];
    }
    dest[0] = (val >> 16) & 0xFF;
    dest[1] = (val >> 8) & 0xFF;
    dest[2] = val & 0xFF;
...
static inline uint32_t get_timestamp_data_element(int idx) {
    const uint8_t *src;
    if (idx < BLE_PART1_ELEMENTS) {
        src = &timestamp_adv_payload_buffer[BLE_PART1_OFFSET + 2 + idx * 3];
    } else if (idx < BLE_PART1_ELEMENTS + BLE_PART2_ELEMENTS) {
        src = &timestamp_adv_payload_buffer[BLE_PART2_OFFSET + 2 + (idx - BLE_PART1_ELEMENTS) * 3];
    } else {
        src = &timestamp_adv_payload_buffer[BLE_PART3_OFFSET + 2 + (idx - BLE_PART1_ELEMENTS - BLE_PART2_ELEMENTS) * 3];
    }
...
static const struct bt_data ad[] = {
    ...
    BT_DATA(BT_DATA_SVC_DATA16, &timestamp_adv_payload_buffer[BLE_PART1_OFFSET], BLE_PART1_BYTE_SIZE),
    BT_DATA(BT_DATA_SVC_DATA16, &timestamp_adv_payload_buffer[BLE_PART2_OFFSET], BLE_PART2_BYTE_SIZE),
    BT_DATA(BT_DATA_SVC_DATA16, &timestamp_adv_payload_buffer[BLE_PART3_OFFSET], BLE_PART3_BYTE_SIZE),
};
...
void main(void)
{
    ...
    memcpy(&timestamp_adv_payload_buffer[BLE_PART1_OFFSET], &uuid, sizeof(uuid));
    memcpy(&timestamp_adv_payload_buffer[BLE_PART2_OFFSET], &uuid, sizeof(uuid));
    memcpy(&timestamp_adv_payload_buffer[BLE_PART3_OFFSET], &uuid, sizeof(uuid));
