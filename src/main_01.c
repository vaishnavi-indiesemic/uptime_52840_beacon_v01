...
static inline void set_uint24_be(uint24_t *dest, uint32_t val) {
    dest->bytes[0] = (val >> 16) & 0xFF;
    dest->bytes[1] = (val >> 8) & 0xFF;
    dest->bytes[2] = val & 0xFF;
}

static inline uint32_t get_uint24_be(const uint24_t *src) {
    return (src->bytes[0] << 16) | (src->bytes[1] << 8) | src->bytes[2];
}
...
#define BLE_DATA_PART_SIZE (BLE_DATA_SIZE / 4)

struct __attribute__((packed)) timestamp_adv_payload_part {
    uint16_t uuid;
    uint24_t data[BLE_DATA_PART_SIZE];
};

struct timestamp_adv_payload_part timestamp_data_part1;
struct timestamp_adv_payload_part timestamp_data_part2;
struct timestamp_adv_payload_part timestamp_data_part3;
struct timestamp_adv_payload_part timestamp_data_part4;

static inline void set_timestamp_data_element(int idx, uint32_t val) {
    if (idx < BLE_DATA_PART_SIZE) {
        set_uint24_be(&timestamp_data_part1.data[idx], val);
    } else if (idx < BLE_DATA_PART_SIZE * 2) {
        set_uint24_be(&timestamp_data_part2.data[idx - BLE_DATA_PART_SIZE], val);
    } else if (idx < BLE_DATA_PART_SIZE * 3) {
        set_uint24_be(&timestamp_data_part3.data[idx - BLE_DATA_PART_SIZE * 2], val);
    } else {
        set_uint24_be(&timestamp_data_part4.data[idx - BLE_DATA_PART_SIZE * 3], val);
    }
}

static inline uint32_t get_timestamp_data_element(int idx) {
    if (idx < BLE_DATA_PART_SIZE) {
        return get_uint24_be(&timestamp_data_part1.data[idx]);
    } else if (idx < BLE_DATA_PART_SIZE * 2) {
        return get_uint24_be(&timestamp_data_part2.data[idx - BLE_DATA_PART_SIZE]);
    } else if (idx < BLE_DATA_PART_SIZE * 3) {
        return get_uint24_be(&timestamp_data_part3.data[idx - BLE_DATA_PART_SIZE * 2]);
    } else {
        return get_uint24_be(&timestamp_data_part4.data[idx - BLE_DATA_PART_SIZE * 3]);
    }
}
...
static const struct bt_data ad[] = {
    ...
    BT_DATA(BT_DATA_SVC_DATA16, &timestamp_data_part1, sizeof(timestamp_data_part1)),
    BT_DATA(BT_DATA_SVC_DATA16, &timestamp_data_part2, sizeof(timestamp_data_part2)),
    BT_DATA(BT_DATA_SVC_DATA16, &timestamp_data_part3, sizeof(timestamp_data_part3)),
    BT_DATA(BT_DATA_SVC_DATA16, &timestamp_data_part4, sizeof(timestamp_data_part4)),
};
...
void main(void)
{
    ...
    timestamp_data_part1.uuid = sys_cpu_to_be16(0xaabb);
    timestamp_data_part2.uuid = sys_cpu_to_be16(0xaabb);
    timestamp_data_part3.uuid = sys_cpu_to_be16(0xaabb);
    timestamp_data_part4.uuid = sys_cpu_to_be16(0xaabb);
